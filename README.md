...
| `import-review` | Turns a reviewer's (human or AI) per-candidate decisions — never derived from static analysis — into ready-to-copy `authMiddleware`/`authWrappers` config entries. Validates every decision against `suggest-auth`'s own exact fingerprints, rejecting anything stale or tampered. Output is always advisory: nothing is ever written to a real `--config` automatically. |
