...
| `import-review` | Turns a reviewer's (human or AI) per-candidate decisions — never derived from static analysis — into ready-to-copy `authMiddleware`/`authWrappers` config entries. Validates every decision against `suggest-auth`'s own exact fingerprints, rejecting anything stale or tampered. With `--out`, also writes a standalone, already-valid config file ready to pass straight to `--config`; nothing is ever merged into an existing `--config` automatically. |
