...
| `fleet` | Runs `audit` once per target listed in a manifest, one remote repository (`--repo`), or a whole GitHub organization (`--org`), aggregating results with bounded concurrency, checkpointed resume, and `--update` to skip unchanged repositories. Organization scans automatically populate an unreviewed per-repository queue under `target-configs-draft/` as each repository finishes; `--suggest-auth` opts manifest/repository scans into the same workflow. Drafts never affect classification. Reviewed `--target-config-dir` files remain separate, are snapshotted durably into `--out`, and are reused on later runs there. See [docs/reference.md](docs/reference.md#fleet-options) and the [scheduled org scan example](examples/github-actions/scheduled-org-scan.yml). |
