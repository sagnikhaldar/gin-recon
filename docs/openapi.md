...
Source-document discovery separately catalogs bounded local OpenAPI 3.x and Swagger 2 JSON/YAML files without overwriting them or fetching remote references. It records dialect/title/version/path/hash/authorship status and reconciles unique documented operations with observed routes. Checked-in does not automatically mean authored; generated documents never count as authored coverage. This catalog is embedded verbatim as the document's own `x-gin-recon-source-specifications` extension, and `api.html` renders it as a dedicated "Source specifications" section (deep-linkable at `api.html#specifications`): every discovered document's title/version/dialect/path/authorship, catalog-level issues, and the code-only/documentation-only/ambiguous-ownership operation lists that reconciliation produced. `fleet.html`'s own per-repository row links straight there instead of repeating that detail inline.
