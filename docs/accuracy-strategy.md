...
- Classifier version-diff tests that run the current and immediately prior release binaries against the frozen corpus and assert identical `proven`/`public`/`unknown`/`enforcementAnalysis` results per route, unless the change is a documented, reviewed widening of [ADR 0008](adr/0008-bounded-enforcement-shape-analysis.md)'s enforcement-shape boundary.
