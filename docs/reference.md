...
- `--render-html`: off by default. `fleet`'s own job is the raw scan; rendering it into HTML is a separate, explicit decision, the same way `render` already treats a saved `fleet.json` — nothing is rendered unless asked (docs/adr/0037-fleet-html-opt-in.md). With it, a sibling **`<out>-html`** directory, derived automatically from `--out` (no separate render step needed once this flag is on) holds every HTML artifact: `fleet.html` at its root and `targets/<name>/api.html` for any target whose own `--format` included `openapi`. A target's `audit` subprocess still runs exactly once; `fleet` moves the resulting `api.html` into this directory afterward, a plain file move rather than a second scan. The self-contained fleet page links to every target's raw report and rendered view, but organizes them into two evidence-based views: a primary table for repositories with observed routes, and a collapsible reference table for zero-route non-Gin, non-Go, failed, inconclusive, or unverified outcomes. Each table has its own search/filter controls, including framework, completion, and API-documentation selectors (`OpenAPI3`, `Swagger2`, `Both`, `None`, `Unavailable`) driven entirely by saved catalogs. The route-bearing table also gets a sort control (routes high/low, unknown count high to low, proven count high to low, or name), reordering rows within their existing completion group rather than across it — the grouping itself is evidence (audit process/coverage quality), not a display accident, so a triage sort never flattens it away; filtering and sorting act independently of each other. Per-module documentation states how many source specification documents were found and shows source-file versus observed-operation coverage separately with counts; incomplete scope remains a warning independent of the percentage. Per-specification detail (title, version, dialect, path, authorship, and a truncated SHA-256/byte-size line so a reviewer with access to the scanned repository can verify they're looking at the exact same file without gin-recon embedding or rendering a copy of it), catalog issues, and the code-only/documentation-only/ambiguous-ownership operation lists all live in that module's own `api.html` — a dedicated "Source specifications" section, deep-linkable at `api.html#specifications` — rather than a per-repository dropdown in `fleet.html` itself; fleet.html links straight to it when the module's own `--format` included `openapi`. Each specification is rendered as its own card; the code-only/documentation-only/ambiguous-ownership lists collapse behind a count badge (open automatically only when short enough to skim), each with a live substring filter and a bounded, scrolling list — a real repository in this fleet had 739 code-only operations, which as a flat bullet list would be unreadable. For a repository with exactly one module, its observed-operation coverage percentage is also shown directly in the collapsed row summary (for example `OpenAPI3 · 25.0% (3/12) documented`), so a reviewer scanning the table doesn't have to expand every row to see how documented a repository actually is; a repository with more than one module still requires expanding, since no single percentage could represent all of its modules at once. A failed or incomplete target with retained route evidence stays in the primary table and keeps its real status and coverage; a process exit of `ok` with zero routes is not called clean unless complete module metadata supports the narrower category shown. The metrics continue to count `ok`, failed, inconclusive, and non-Go audit statuses independently of this presentation grouping, and every raw target remains in `fleet.json`. A metric tile and an overview line report fleet-wide OpenAPI/Swagger documentation coverage — how many specification documents were found in total, and what percentage of successfully-scanned repositories carry at least one — recomputed from every target's own already-decoded `specifications` field rather than read from a single rollup, so it stays correct even against an older `fleet.json` re-rendered by a newer `gin-recon`. A second overview line answers the route-level question that repository-level metric can't: summed straight from every module's own already-computed `observedOperations` metric, what fraction of every observed route across the entire fleet is actually matched to a discovered specification, not just how many repositories have some specification file. A repository whose one specification covers a sliver of its routes still counts toward the first metric; only this second one reflects that. These overview rows are grouped under two small headings, "Authentication evidence" and "Documentation evidence", so the two concerns read as separate at a glance rather than one undifferentiated stack. An `--org` page also gets a Scope panel (organization, cap, concurrency, archived/forks, repo filters), labels the target count as selected rather than discovered, and lists filtered/skipped/capped discovery dispositions separately from audit outcomes. Its metrics and tables surface each target's own `proven`/`public`/`unknown` evidence counts, copied from that target's own `routes.json` `summary` — no second scan (docs/adr/0029-fleet-html-evidence-dashboard.md). A "Configuration" panel and, for an `--org` run, an "Enumeration coverage" row report whether this run's `--config` actually named any `authMiddleware`/`authWrappers` at all — with none configured, every route structurally defaults to `public`/`unknown`, and the page says so directly rather than leaving a fleet-wide `Proven` of zero looking like a detection failure (docs/adr/0030-fleet-html-auth-config-visibility.md). The Configuration panel also reports whether this run's `--config` set `analysis.followModules` at all — without it, a service that imports and mounts another module's own routes (a real, common pattern: one service calling another module's own `Init(router, ...)`) won't have those routes counted, a second silently-narrowing config knob alongside `authMiddleware` (docs/adr/0036-fleet-html-follow-modules-visibility.md).
...
`fleet.json.repositoryGroups` is an additive, derived index over the unchanged `targets` array. `withRoutes` contains sorted target names with observed routes. `reference` contains sorted `{name, category}` entries using `gin-no-routes`, `no-routes-unverified`, `non-gin`, `not-go-module`, `failed`, or `inconclusive`. Only a complete successful target with complete module metadata is categorized as Gin-without-routes or non-Gin; incomplete and metadata-free zero-route results stay unverified. A target with routes always remains in `withRoutes`, even if a later module made its overall status failed or incomplete. The index never controls scanning, authentication classification, coverage, resume, or update reuse. It is optional in schema 1.0 so older fleet files remain renderable; offline render recomputes and writes it from the refreshed targets.
...
- `--include-archived` / `--include-forks`: both default to excluded. Passing either is an explicit scope expansion; the selected repositories are preserved, and `fleet.html` calls the opt-in out rather than silently dropping them because a later reviewer would have preferred the defaults. A disabled or genuinely empty (zero content) repository is always excluded, regardless of these flags — there is nothing a clone could do with either.
...
`import-review --bundle <suggestions.json> [--assessment <assessment.json>]` turns a reviewer's own decisions about `suggest-auth`'s candidates into ready-to-copy `authMiddleware`/`authWrappers` config entries — it never merges into an existing `--config` file, and it never runs analysis (no `--src`, no source tree). Writes JSON to stdout unless `--out` is given, in which case it writes two files (`--force` required to overwrite either): `<out>/review-suggestions.json`, the full advisory document with a per-candidate decision record, and `<out>/reviewed-config.json`, a second, standalone, already-valid `{"version": 1, "authMiddleware": {...}, "authWrappers": [...]}` document — pass it straight to `audit`/`fleet`'s own `--config` as-is, or merge its entries into an existing config by hand. The standalone file is never written when `--out` is omitted (stdout mode only ever emits the one advisory document).
...
- `--assessment` is optional. Given, it is a JSON document a human or an AI reviewer writes: `{"schemaVersion": "1.0", "bundleFingerprint": "<copied from the bundle>", "decisions": [...]}`. Each decision names a `candidateId`/`candidateFingerprint` (copied from the exact candidate it assessed), `isAuthGuard` (the reviewer's own judgment — never derived from `nameHint` or `enforcementShape`), and — only when `isAuthGuard` is true — an `assurance` (`analyze` or `attested`, the same two config modes [Canonical symbols and assurance](#canonical-symbols-and-assurance) documents) plus optional `tags`/`roles`/`scopes`/`openapiScheme`/`transparentWrapper`. `rationale` is required on every decision, so a decision is never a bare yes/no a later reader has no way to check. Omitted, `docs/adr/0042-static-analysis-drafts-assessments.md` applies: gin-recon drafts one itself — `isAuthGuard: true, assurance: analyze` for every candidate where `nameHint` *and* `enforcementShape: confirmed-shape` both hold (empirically validated across 39 real repositories with zero false positives; neither signal alone), leaving every other candidate out of the draft entirely rather than guessing. Every drafted decision's `rationale` says plainly that it was machine-generated, not human- or AI-verified — a human or AI can still review, extend, or override anything by passing `--assessment` explicitly.
...
A route optionally carries `swag`: source-aware evidence parsed from the bounded swaggo/swag dialect documented in [OpenAPI documentation](openapi.md#swaggoswag-doc-comment-annotations). It includes prose, repeated router claims, request parameters/body/form data, responses/headers, content types, schemas, extensions, and documentary security. It never affects route identity or auth classification. Named types unresolved in syntax-only/typed analysis remain explicit instead of becoming empty schemas.

Reports may also carry module-scoped `documentation` (global annotations) and `specifications` (the bounded offline OpenAPI 3.x/Swagger 2 catalog and reconciliation metrics). `specifications.metrics.sourceFiles` and `.observedOperations` retain numerator, denominator, and completeness independently; zero or missing denominators display as N/A, and `incompleteScope` is never hidden by a percentage. Fleet results retain the same catalog under each module scope so saved HTML can filter and render without rescanning. `--format md`/pretty text render the same catalog as a "Documentation"/"DOCUMENTATION" section: specification count and title/dialect/version/path per document, the same source-file/observed-operation coverage percentages, and an explicit statement when no specification was discovered at all.
