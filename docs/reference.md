...
- Progress: one `[N/total] <name>: <status>` line per target is printed to stderr live as each target finishes (or is reused from a `--resume` checkpoint, marked `(resumed)`) — not batched up until the whole run exits, since a fleet run against a real organization can take minutes with nothing else visible otherwise. Always on, no flag; separate from the buffered per-target error tails printed as one block once the run completes (docs/adr/0032-fleet-progress-reporting.md).
...
- `--update` (`--org` only): skip a target whose GitHub `pushedAt` hasn't changed since the *previous complete run* at this same `--out` — a different thing from `--resume`, which only reuses a target already finished earlier in the *same* run. A target with no prior complete-run result, or whose `pushedAt` moved, is scanned normally; every reuse is refused if the previous run's `toolVersion` differs from this binary's. Progress logs `(unchanged)` for a reused target (distinct from `--resume`'s own `(resumed)`); `fleet.json`'s `update.reused` counts them. `--update` cannot be combined with `--resume` or `--force` — each is already a complete answer to "output already exists here" — and, like them, bypasses the already-exists conflict check/prompt entirely (docs/adr/0039-fleet-org-update.md).
