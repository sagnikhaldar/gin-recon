...
- `--out <dir>` (required): the **raw** artifacts root — `fleet.json` (the aggregate), `fleet-delta.json` (with `--baseline`), `discovered-targets.json` (with `--org`), and each target's own JSON-format output under `targets/<name>/`. No HTML file is ever written here.
