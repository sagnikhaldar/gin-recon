...
- `--suggest-auth`: off by default. Every successfully-scanned target also runs `suggest-auth` (writing `suggestions.json` alongside that target's own `routes.json`), and `fleet` writes **`fleet-auth-candidates.json`** at `--out` — every target's candidates merged by canonical symbol into one organization-wide ranked list, so building a reviewed `authMiddleware` config for a whole organization means reading one document instead of running `suggest-auth` by hand against each repository. Each entry reports `routeCount` (summed across every target where the symbol appears), `repoCount` and `repos` (which targets use it — capped at 20 names, with `repoCountTotal` set when the real count is higher), `nameHint`, `knownNonAuth`, and up to 5 labeled `sampleRoutes` (`"<target>: METHOD /path"`). Ranked by `repoCount` descending, then `routeCount` descending — a symbol reused across many repositories (a shared internal auth package) is usually the strongest signal worth reviewing first. A target whose own `suggest-auth` pass fails, or that was reused rather than freshly scanned (`--update`/`--resume`), simply contributes nothing to the aggregate — never a reason to fail the run. Exactly like `suggest-auth` itself, this never affects classification; every candidate is still for a human/AI reviewer to check against real source before adding to `authMiddleware`.
...
- `--progress auto|plain|json|none`: default `plain`. Plain mode emits one live `[N/total] <name>: <status>` line per target; JSON mode emits one bounded JSON object per line with status, reuse reason, coverage, routes, attempts, and duration. `auto` selects plain on a terminal and JSON otherwise; `none` disables progress.
